...
new Vue({
  ...
  methods: {
    ...
    filterConfirmedOrder: function filterConfirmedOrder(yearAndMonth) {
      ...
      axios.get("/admin/chart/daily_order/".concat(y, "/").concat(m)).then(function (response) {
        if (response.data.success) {
          // hideLoading();
          _this.OrderChart(response.data.data.total, "daily_order");
    ...
    filterPendingOrder: function filterPendingOrder(yearAndMonth) {
      ...
      var y = monthYear[0];
      var m = monthYear[1];
      axios.get("admin/chart/monthly_order/".concat(y)).then(function (response) {
        console.log(response);
        if (response.data.success) {
          // hideLoading();
          _this2.OrderChart(response.data.data.total, "monthly_order");
          console.log(response.data.data.total);
        } else {
          alert(response.data.message);
          // showAlertError(response.data.message);
  ...
  mounted: function mounted() {
    ...
    $("#filter_date_confirmed_order").on("change.datetimepicker", function (e) {
      _this3.filterConfirmedOrder($('#filter_date_confirmed_order').val());
    });
    ...
    $("#filter_date_pending_order").on("change.datetimepicker", function (e) {
      _this3.filterPendingOrder($('#filter_date_pending_order').val());
    });
...
module.exports = __webpack_require__(/*! C:\Users\Sengs\OneDrive\Desktop\laravelvid\laravel-foodmenu\admin\resources\assets\js\chart\chart.js */"./resources/assets/js/chart/chart.js");
