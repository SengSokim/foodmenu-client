...
new Vue({
  ...
  methods: {
    ...
    save: function save() {
      ...
      axios.post("/portal/product_variants/".concat((_this$data$id = this.data.id) !== null && _this$data$id !== void 0 ? _this$data$id : ''), this.data).then(function (response) {
        if (response.data.success) {
          window.location.href = '/portal/product_variants?product_id=' + _this.data.product_id;
        } else {
    ...
    deleteVariant: function deleteVariant() {
      ...
      showLoading();
      axios["delete"]("/portal/product_variants/".concat(this.data.id)).then(function (response) {
        hideLoading();
        console.log(response.data);

        if (response.data.success) {
          console.log(_this4.data);
          window.location.href = '/portal/product_variants?product_id=' + _this4.data.product_id;
        } else {
          showAlertError(response.data.message);
        }
      })["catch"](function (error) {
...
module.exports = __webpack_require__(/*! C:\wamp64\www\Work Project\emenu\merchant\resources\assets\js\product-variants\product-variant.js */"./resources/assets/js/product-variants/product-variant.js");
