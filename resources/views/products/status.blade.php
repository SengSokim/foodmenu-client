    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <form @submit.prevent="deleteProduct">
          ...
          <button type="button" v-if="data.enable_status == 0" @click="updateProductStatus(1)" class="btn btn-warning btn-sm rounded-pill">{{ __('app.product.active') }}</button>
          <button type="button" v-else @click="updateProductStatus(0)" class="btn btn-warning btn-sm rounded-pill">{{ __('app.global.deactive') }}</button>
        </form>
      </div>
    </div>
  </div>
